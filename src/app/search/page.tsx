  ...
  return (
    <div>
      <div className="max-h-64 overflow-auto">
        {pricingOptionsForProduct.map((pricing) => (
          <div className="flex border-b border-slate-300 py-2" key={pricing.id}>
            <Image
              ...
              alt="Store logo"
            />
            <div className="flex-1 text-right">${pricing.price}</div>
          </div>
        ))}
      </div>
      {pricingOptionsForProduct.length > 0 ? (
        <div className="mt-2 flex justify-end">
          <button className="mb-2 rounded-lg bg-green-700 px-3 py-1.5 text-sm font-medium text-white hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
            Add to Cart
          </button>
        </div>
      ) : (
